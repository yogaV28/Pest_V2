...
class DetectionModel(paddle.nn.Layer):
    def __init__(self):
        ...
        self.conv2d50 = paddle.nn.Conv2D(out_channels=5, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d56 = paddle.nn.Conv2D(out_channels=5, kernel_size=(1, 1), in_channels=64)
        ...
        self.conv2d62 = paddle.nn.Conv2D(out_channels=5, kernel_size=(1, 1), in_channels=64)
    ...
    def forward(self, x0):
        ...
        x830 = paddle.reshape(x=x712, shape=[1, 69, -1])
        x832 = paddle.reshape(x=x770, shape=[1, 69, -1])
        x834 = paddle.reshape(x=x828, shape=[1, 69, -1])
        x835 = [x830, x832, x834]
        x836 = paddle.concat(x=x835, axis=2)
        x838 = paddle.split(x=x836, num_or_sections=[64, 5], axis=1)
        x839, x840 = x838
...
def main(x0):
    ...
    params = paddle.load(r'C:\Users\yogav\Documents\Project\Yoga_project\nvidia project\PEct_tensorrt\best_paddle_model\model.pdparams')
